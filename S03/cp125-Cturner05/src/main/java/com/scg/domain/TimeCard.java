...
public final class TimeCard implements Comparable<TimeCard>, Serializable {
    ...
    /**
     * Compares TimeCard, by ascending date order, consultant, totalHours,
    ...
    public int compareTo(final TimeCard other) {
        ...
        if (this.totalHours < other.totalHours) return BEFORE;
        if (this.totalHours > other.totalHours) return AFTER;

        if (this.totalBillableHours < other.totalBillableHours) return BEFORE;
        if (this.totalBillableHours > other.totalBillableHours) return AFTER;

        if (this.totalNonBillableHours < other.totalNonBillableHours) return BEFORE;
        if (this.totalNonBillableHours > other.totalNonBillableHours) return AFTER;

        // At this point we could walk through the list and compare the individual
        // ConsultantTime entries, but, its pretty expensive so we skip it.
