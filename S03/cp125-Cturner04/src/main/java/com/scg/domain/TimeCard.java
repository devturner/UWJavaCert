...
public final class TimeCard implements Comparable<TimeCard> {
	...
	private static final String LINE_HEADER_FORMAT = String
			.format("%-28s %-10s  %5s  %s%n"
					+ "---------------------------  ----------  -----  --------------------%n",
	 ...
	 *            records information for; assumed to be a Monday.
	 */
	 ...
	 */
	public List<ConsultantTime> getBillableHoursForClient(
	 ...
	 *
	 * @param formatter
	 *            the formatter to add the lines to
	 * @param hours
	 *            the list of consulting hours
	 * @param billable
	 *            if true billable hours will be added otherwise non-billable
	 */
	private void appendTime(final Formatter formatter,
			final List<ConsultantTime> hours, final boolean billable) {
		for (final ConsultantTime currentTime : hours) {
			if (currentTime.isBillable() == billable) {
				formatter.format(LINE_FORMAT, currentTime.getAccount()
						.getName(), currentTime.getDate(), currentTime
						.getHours(), currentTime.getSkill());
			}
		}
	}
	...
	public String toString() {
		return String.format(TO_STRING_FORMAT, consultant.getName(),
				weekStartingDay);
	}
	...
	public String toReportString() {
		...
		formatter.format(NON_BILLABLE_TIME_HEADER_FORMAT).format(
				LINE_HEADER_FORMAT);

		appendTime(formatter, consultingHours, false);

		formatter
				.format(SUMMARY_HEADER_FORMAT)
	...
	/**
	 * This is the TimeCard compare to: ascending date order, consultant,
	 * totalHours, totalBillableHours and totalNonBillableHours.
	 */
	@Override
	public int compareTo(TimeCard other) {
		int diff;
		diff = weekStartingDay.compareTo(other.weekStartingDay);
		if (diff == 0) {
			diff = consultant.compareTo(other.consultant);
			if (diff == 0) {
				int a = this.getTotalHours();
				int b = other.getTotalHours();
				diff = ((a < b) ? 1 : (a > b) ? -1 : 0);
				if (diff != 0) {
					return diff;
				}
				if (diff == 0) {
					int c = this.totalBillableHours;
					int d = other.totalBillableHours;
					diff = ((c < d) ? 1 : (a > b) ? -1 : 0);
					if (diff != 0) {
						return diff;
					}
					if (diff == 0) {
						int e = this.totalNonBillableHours;
						int f = other.totalNonBillableHours;
						diff = ((e < f) ? 1 : (e > f) ? -1 : 0);
